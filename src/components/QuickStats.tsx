...
const QuickStats: React.FC<QuickStatsProps> = ({ dict }) => {
    ...
    return (
        <div className="py-12 my-8 bg-gradient-to-br from-accent-gold/5 to-orange-400/5 rounded-2xl">
            <h2 className="font-heading text-3xl md:text-4xl text-foreground text-center mb-12">{dict.title}</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 max-w-[1200px] mx-auto px-8">
                {stats.map((stat, index) => (
                    <div key={index} className="text-center p-8 bg-card-bg rounded-xl border-2 border-transparent transition-all duration-300 hover:-translate-y-1 hover:border-accent-gold hover:shadow-lg">
                        <div className="text-5xl mb-4">{stat.emoji}</div>
                        <div className="text-4xl md:text-[2.5rem] font-extrabold text-[#ff4d4d] font-heading mb-2">{stat.value}</div>
